      ...
      <div className="relative p-5">
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1 grid grid-cols-2 gap-2">
            <div className="text-center py-2.5 rounded-xl bg-cyan-500/[0.06]">
...
export default function Districts() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [d, p] = await Promise.all([api.get('/api/districts'), api.get('/api/provinces')]);
      setItems(d); setProvinces(p);
    } catch {}
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const remove = async (id) => {
    ...
    try { await api.del(`/api/districts/${id}`); load(); } catch (e) { alert(e.message); }
  };
    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{items.length} ta tuman · {totalSchools} maktab</p>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>{filtered.map((item, i) => (
              <tr key={item.id} className="border-b border-emerald-500/[0.04] hover:bg-emerald-500/[0.03] transition-colors">
                ...
                <td className="px-5 py-3 text-slate-400">{item.provinceName}</td>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Tahrirlash' : 'Yangi tuman'}>
        <Input label="Tuman nomi" value={form.name || ''} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Masalan: Chilonzor" />
        <Select label="Viloyat" value={form.provinceId || ''} onChange={e => setForm({ ...form, provinceId: e.target.value })}
