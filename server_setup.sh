...
sudo -u postgres psql -c "ALTER USER postgres PASSWORD '***PURGED_PASSWORD***';" 2>/dev/null
...
pm2 start "java -jar target/backend-0.0.1-SNAPSHOT.jar --spring.datasource.url=jdbc:postgresql://localhost:5432/maktabdb --spring.datasource.username=postgres --spring.datasource.password=***PURGED_PASSWORD*** --spring.jpa.hibernate.ddl-auto=update --server.port=8081" --name "maktab-backend"

# Gilam Backend
cd /home/team/gilam-platforma/backend
DATABASE_URL="postgresql://postgres:***PURGED_PASSWORD***@localhost:5432/gilamdb" PORT=3001 NODE_ENV=production pm2 start index.js --name "gilam-backend"
